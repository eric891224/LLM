...
from engine.event_timer import CrossNodeEventTimerV3
...
class MixtralAttentionTP(nn.Module):
    ...
    def __init__(
        ...
        timer: CrossNodeEventTimerV3 = None
    ):
        super().__init__()
        self.config = config
...
class MixtralSparseMoeBlockTP(nn.Module):
    ...
    def __init__(self, config, tp_group, timer: CrossNodeEventTimerV3):
        super().__init__()
...
class MixtralDecoderLayerTP(nn.Module):
    def __init__(
        ...
        layer_idx: int,
        tp_group: dist.distributed_c10d.ProcessGroup,
        timer: CrossNodeEventTimerV3
    ):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = MIXTRAL_ATTENTION_CLASSES[config._attn_implementation](
            config, layer_idx, tp_group, timer
        )

        self.block_sparse_moe = MixtralSparseMoeBlockTP(config, tp_group, timer)
