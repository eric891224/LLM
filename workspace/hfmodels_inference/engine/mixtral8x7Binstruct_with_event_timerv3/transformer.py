...
timer = CrossNodeEventTimerV3(local_rank=LOCAL_RANK, world_size=WORLD_SIZE, world_rank=WORLD_RANK)
    ...
    def __init__(
        ...
        if self.num_tp_ranks == 1:
            ...
            if num_ep_ranks != None:
                ...
                layers = [
                    MixtralDecoderLayerTP(config, layer_idx, self.tp_group, timer)
                    for layer_idx in range(config.num_hidden_layers)
                ]
